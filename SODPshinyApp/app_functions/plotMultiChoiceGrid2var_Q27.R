...
# this function is for plotting the multiple choice grid (table) question Q27 combined with a demographic.
...
#questionName <- "When you share your data, how much do you make open?"
#demographic <- "Age"

plotMultiChoiceGrid2var_Q27 <- function(questionName, demographic, data_key, data_all, lookupTable) {

   questionName <- as.character(lookupTable %>% filter(question == questionName) %>% select(ques_level1))
   
   # Extract the Question answers and key from data_all and data_key and draw a plot: there are THREE levels
   ...
   mcg_answers_a <- answers_mcg %>% select("URN", contains("a")) # none
   mcg_answers_b <- answers_mcg %>% select("URN", contains("b")) # some
   mcg_answers_c <- answers_mcg %>% select("URN", contains("c")) # all
   ...
   name_one <- mcg_key[[1]][1,3]    # 'c' - all
   name_two <- mcg_key[[2]][1,3]    # 'a' - none
   name_three <- mcg_key[[3]][1,3]  # 'b' - some

   # rename the column headings containing ques nos in answers with the choice options text
   colnames(mcg_answers_a) <- c("URN", mcg_key[[2]]$choice_options_text)
   colnames(mcg_answers_b) <- c("URN", mcg_key[[3]]$choice_options_text)
   colnames(mcg_answers_c) <- c("URN", mcg_key[[1]]$choice_options_text)
   ...
   # join the key/choice options text to answers files a, b, c using URN numbers ====== ##
   # ==================
   ...
   grid_arrange_shared_legend(p1, p2, p3, ncol = 1, nrow = 3)  # shares the legend
     
   }

#plotMultiChoiceGrid2var_Q27("When you share your data, how much do you make open?", "Age")
