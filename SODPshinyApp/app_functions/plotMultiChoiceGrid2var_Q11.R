...
plotMultiChoiceGrid2var_Q11 <- function(questionName, demographic) {
   ...
   mcg_answers_a <- answers_mcg %>% select("URN", contains("a")) # during research
   mcg_answers_b <- answers_mcg %>% select("URN", contains("b")) # after research
   ...
   name_one <- mcg_key[[1]][1,3] # 'b's or After research
   name_two <- mcg_key[[2]][1,3] # 'a's or During research
   ...
   grid_arrange_shared_legend(p1, arrangeGrob(p2+nt, ncol=1), ncol=1, nrow=2) # shares the legend
   
   }

#plotMultiChoiceGrid2var_Q11("Where did you store the data you created while you were conducting your research and after research was completed (select all that apply)", "Age")
