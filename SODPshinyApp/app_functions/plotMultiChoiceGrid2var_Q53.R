...
# this function is for plotting the multiple choice grid (table) question Q53 combined with a demographic.
...
plotMultiChoiceGrid2var_Q53 <- function(questionName, demographic,  data_key, data_all, lookupTable) {
   ...
   # Extract the Question answers and key from data_all and data_key and draw a plot: there are TWO levels
   ...
   mcg_answers_a <- answers_mcg %>% select("URN", contains("a")) # before publication
   mcg_answers_b <- answers_mcg %>% select("URN", contains("b")) # after publication
   ...
   name_one <- mcg_key[[1]][1,3] # 'b's or after publication
   name_two <- mcg_key[[2]][1,3] # 'a's or before publication
   ...
   grid_arrange_shared_legend(p1, p2, ncol = 1, nrow = 2)  # shares the legend
   
   }

#plotMultiChoiceGrid2var_Q53("Who do you think owns the data you have produced in your last research project? Does this differ before publication and after publication?", "Age")
