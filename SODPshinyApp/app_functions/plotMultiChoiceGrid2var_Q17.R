...
# this function is for plotting the multiple choice grid (table) question Q17 combined with a demographic.
...
plotMultiChoiceGrid2var_Q17 <- function(questionName, demographic) {
   ...
   mcg_answers_a <- answers_mcg %>% select("URN", contains("a")) # immediately on creation
   mcg_answers_b <- answers_mcg %>% select("URN", contains("b")) # on publication
   mcg_answers_c <- answers_mcg %>% select("URN", contains("c")) # after project completion
   mcg_answers_d <- answers_mcg %>% select("URN", contains("d")) # I do not share these data
   ...
   name_one <- mcg_key[[1]][1,3]    # 'c' - after project completion
   name_two <- mcg_key[[2]][1,3]    # 'd' - I do not share these data
   name_three <- mcg_key[[3]][1,3]  # 'a' - immediately on creation
   name_four <- mcg_key[[4]][1,3]   # 'b' - on publication
   ...
   colnames(mcg_answers_b) <- c("URN", mcg_key[[4]]$choice_options_text)
   colnames(mcg_answers_c) <- c("URN", mcg_key[[1]]$choice_options_text)
   colnames(mcg_answers_d) <- c("URN", mcg_key[[2]]$choice_options_text)
   ...
   p4 <- ggplot(df_plot_d, aes(x=counts, y=option, fill=choice_options_text)) +
         geom_bar(stat = "identity") +
         labs(title = name_two, x = "Count ", y = "", fill = " ") +
         theme(text = element_text(size = 11), plot.title = element_text(size = 12)) +
         guides(fill = guide_legend(ncol = 3, byrow = TRUE)) +
         coord_cartesian(xlim =c(0, 35))
...
#plotMultiChoiceGrid2var_Q17("At what point do you start sharing different types of data?)", "Age")
