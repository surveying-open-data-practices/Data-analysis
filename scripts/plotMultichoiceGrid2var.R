...
plotMultichoiceGrid2var <- function(questionName) {
...
answers_demographic <- data_all %>% select(URN, "Age") # for function replace Q1 with demographic

## get question key for the demographic and left join onto the answers_demographic
# question key for the demographic
question_key_demographic <- data_key %>% filter(demographicCategory == "Age") %>%
